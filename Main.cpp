...
void __fastcall TfrMain::btnSortListBoxClick(TObject *Sender)
{
	...
	btnInstantiate->Visible = true;
	 ...
	 for(int x=0;x< lstInfo->Items->Count;x++)
	  {
			  line= lstInfo->Items->Strings[x];
			  line.Delete(1,line.Pos("\t"));
				newLine = IntToStr(x+1)+'\t'+line;
			lstInfo->Items->Strings[x] = newLine;
	 }
	}
//---------------------------------------------------------------------------
...
void __fastcall TfrMain::btnSearchClick(TObject *Sender)
{
	AnsiString Ln,Id,ini,surname,fileNo,condition ,gender;
	int genderNo;
	 AnsiString search ;
	for(int i=0;i<lstInfo->Count;i++)
	{
		...
		if (edtSearch->Text == "")
		{
				MessageDlg("Please enter the file number before you can search !!! ", mtInformation, TMsgDlgButtons() << mbOK, 0);
				return;
		}
		else
		{
			...
			for (int i = 0; i < lstInfo->Items->Count; i++)
			{
				...
				if (search == fileNo)
				{
					ShowMessage("File Number found at " + IntToStr(i + 1));
					found = true;
					lstInfo->ItemIndex = i;
					return;
			...
			if (!found)
			{
				MessageDlg("Patient not found", mtError, TMsgDlgButtons() << mbOK, 0);
				return;
			}
		}
	}
